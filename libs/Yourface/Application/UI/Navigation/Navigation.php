...
class Navigation extends Control
{
	...
	{
                $node = $this->getComponent("homepage")->add($label, $url);
		...
		$template = $this->createTemplate()
			->setFile(dirname(__FILE__) . "/menu.phtml");
                ...
                if (\Nette\Environment::getContext()->hasService('translator')) {
                    $template->setTranslator(\Nette\Environment::getService('translator'));
                }
	...
	public function renderBreadcrumbs()
	{
        ...
        public function hasItems() {
            return $this->current && count($this->current->getComponents()) ? true : false;
        }
}
